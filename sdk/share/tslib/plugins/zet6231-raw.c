...
static void _tpInitSpec_vendor(void)
{
    ...
    gTpSpec.tpI2cDeviceId       = (char)0x76;	              //from this driver setting
...
static void _tpUpdate(struct ts_sample *tpSmp)
{
    if(g_IsTpInitialized!=true)
    {
        ...
        ithDelay(TP_IDLE_TIME_NO_INITIAL);
        return;
    }
    ...
    ithDelay(TP_IDLE_TIME);	//sleep 2ms
}
 ...
 */ 
static int zet6231_read(struct tslib_module_info *inf, struct ts_sample *samp, int nr)
{
...
static const struct tslib_ops zet6231_ops =
{
	zet6231_read,
};
