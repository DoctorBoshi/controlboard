 ...
 ****************************************************************************/
#define TP_I2C_DEVICE_ID       (0xE0>>1)
...
static void _tpInitSpec_vendor(void)
{
    ...
    gTpSpec.tpCvtSwapXY        = (char)0;                    //from Kconfig setting
    ...
    gTpSpec.tpReadChipRegCnt    = (int)3;
...
static int _tpReadPointBuffer_vendor(unsigned char *buf, int cnt)
{
	...
	ft5xxx_evt = alloca(sizeof(*ft5xxx_evt));
	
	I2cCmd = 0x12;		//1100 0010
	ft5xxx_evt->slaveAddress   = TP_I2C_DEVICE_ID;
	ft5xxx_evt->cmdBuffer      = &I2cCmd;
	ft5xxx_evt->cmdBufferSize  = 1;
	ft5xxx_evt->dataBuffer     = buf;
	ft5xxx_evt->dataBufferSize = cnt;
	i2cret = read(gTpInfo.tpDevFd, ft5xxx_evt, 1);
...
static int _tpParseRawPxy_vendor(struct ts_sample *s, unsigned char *buf)
{
	if( buf[0] & 0x80 )
	{
		...
		{
			s->x = (int)((((unsigned int)buf[0]<<4)&0x0700) | ((unsigned int)buf[1]));
			s->y = (int)((((unsigned int)buf[0]<<8)&0x0700) | ((unsigned int)buf[2]));
			s->pressure = 1;
		}
...
static void _tpIntNotActiveRule_vendor(struct ts_sample *tpSmp)
{
    ...
    {
        /*
        _tpDoZt2083ActiveRule();
        1.pull low
        2.sample data rate
        3.have to send 1st event
        4.INT will not auto clear
        5.
        */
        return;
    }
