                    ...
                    else
                    {
                        ...
                        if (surf)
                        {
                            ...
                            case ITU_TRANSFORM_TURN_LEFT:
                                ituTransformBlt(surf, 0, 0, icon->surf, 0, 0, icon->surf->width, icon->surf->height, w, h, icon->surf->width - w, 0, icon->surf->width - w, icon->surf->height, w, icon->surf->height - h, true, ITU_PAGEFLOW_FOLD2);
                                break;

                            case ITU_TRANSFORM_TURN_TOP:
                                ituTransformBlt(surf, 0, 0, icon->surf, 0, 0, icon->surf->width, icon->surf->height, w, h, icon->surf->width - w, h, icon->surf->width, icon->surf->height - h, 0, icon->surf->height - h, true, ITU_PAGEFLOW_FOLD2);
                                break;

                            case ITU_TRANSFORM_TURN_RIGHT:
                                ituTransformBlt(surf, 0, 0, icon->surf, 0, 0, icon->surf->width, icon->surf->height, w, 0, icon->surf->width - w, h, icon->surf->width - w, icon->surf->height - h, w, icon->surf->height, false, ITU_PAGEFLOW_FOLD2);
                                break;

                            case ITU_TRANSFORM_TURN_BOTTOM:
                                ituTransformBlt(surf, 0, 0, icon->surf, 0, 0, icon->surf->width, icon->surf->height, 0, h, icon->surf->width, h, icon->surf->width - w, icon->surf->height - h, w, icon->surf->height - h, true, ITU_PAGEFLOW_FOLD2);
                                break;
                            }
            ...
            else
            {
                if (widget->angle == 0)
                {
                    ...
                    else
                    {
                        ...
                        {
                        case ITU_TRANSFORM_TURN_LEFT:
                            ituTransformBlt(dest, destx, desty, icon->surf, 0, 0, rect->width, rect->height, w, h, rect->width - w, 0, rect->width - w, rect->height, w, rect->height - h, true, ITU_PAGEFLOW_FOLD2);
                            break;

                        case ITU_TRANSFORM_TURN_TOP:
                            ituTransformBlt(dest, destx, desty, icon->surf, 0, 0, rect->width, rect->height, w, h, rect->width - w, h, rect->width, rect->height - h, 0, rect->height - h, true, ITU_PAGEFLOW_FOLD2);
                            break;

                        case ITU_TRANSFORM_TURN_RIGHT:
                            ituTransformBlt(dest, destx, desty, icon->surf, 0, 0, rect->width, rect->height, w, 0, rect->width - w, h, rect->width - w, rect->height - h, w, rect->height, false, ITU_PAGEFLOW_FOLD2);
                            break;

                        case ITU_TRANSFORM_TURN_BOTTOM:
                            ituTransformBlt(dest, destx, desty, icon->surf, 0, 0, rect->width, rect->height, 0, h, rect->width, h, rect->width - w, rect->height - h, w, rect->height - h, true, ITU_PAGEFLOW_FOLD2);
                            break;
                        }
                    }
                }
...
void ituIconLoadJpegData(ITUIcon* icon, uint8_t* data, int size)
{
    ...
    else
    {
        ...
        if (widget->flags & ITU_FIT_TO_RECT)
            surf = ituJpegLoad(rect->width, rect->height, data, size);
        else
