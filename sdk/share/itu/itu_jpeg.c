    ...
    uint16_t        *dest)
{
	...
	if( (width == ithLcdGetHeight()) && (height ==	ithLcdGetWidth()) )
	{			
		if (width > ithLcdGetHeight())
			width = ithLcdGetHeight();
		if (height > ithLcdGetWidth())
			height = ithLcdGetWidth();
	}else
	{
		if (width > ithLcdGetWidth())
			width = ithLcdGetWidth();
		if (height > ithLcdGetHeight())
    ...
    outInfo.width    = destRect->w; //dispWidth;
    outInfo.height   = destRect->h; //dispHeight;
...
int *ituJpegLoadEx(int width, int height, uint8_t *data, int size)
{
	...
	while( (new_index = itv_get_vidSurf_index()) < 0)
	{
    ...
    if (imgWidth * imgHeight >= MAX_JPEG_DECODE_SIZE)
    {
...
printf("+ituJpegLoad(%d,%d,0x%X,%d)\n", width, height, data, size);
    surf = ituJpegLoad(width, height, data, size);
printf("-ituJpegLoad\n");
end:
    free(data);
