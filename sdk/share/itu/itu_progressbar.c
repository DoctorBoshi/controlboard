    ...
    else if (desta > 0)
    {
        ...
        if (surf)
        {
            if (bar->layout == ITU_LAYOUT_HORIZONTAL)
            {
                ...
                if (width > 0)
                {
                    ituSurfaceSetClipping(dest, prevClip.x, prevClip.y, destx + width, prevClip.height);
            ...
            else //if (bar->layout == ITU_LAYOUT_VERTICAL)
            {
                ...
                if (height > 0)
                {
                    ituSurfaceSetClipping(dest, prevClip.x, desty + rect->height - height, prevClip.width, prevClip.height);

                    if (bg->graidentMode == ITU_GF_NONE)
                        ituColorFill(surf, 0, 0, rect->width, height, &bar->fgColor);
                    else
                        ituGradientFill(surf, 0, 0, rect->width, rect->height, &bar->fgColor, &bg->graidentColor, bg->graidentMode);
                }
            }
