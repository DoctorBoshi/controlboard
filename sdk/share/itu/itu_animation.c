            ...
            if (--animation->delayCount <= 0)
            {
                ...
                else
                {
                    ...
                    if (target)
                    {
                        ...
                        {
                            if (!(animation->animationFlags & ITU_ANIM_MOVE) && (animation->animationFlags & ITU_ANIM_SCALE_CENTER))
                            {
                                animation->child->rect.x = animation->keyRect.x - (target->rect.width - animation->keyRect.width) * frame / animation->totalframe;
                                animation->child->rect.y = animation->keyRect.x - (target->rect.height - animation->keyRect.height) * frame / animation->totalframe;
...
void ituAnimationGotoKeyframe(ITUAnimation* animation, int keyframe)
{
    ...
    if (target)
    {
        ...
        if ((animation->animationFlags & ITU_ANIM_MOVE) == 0)
        {
