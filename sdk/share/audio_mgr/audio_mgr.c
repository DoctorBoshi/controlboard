...
#define MULTISECTION_SIZE       (64*1024)
    ...
    for (;;)
    {
        ...
        for (;;)
        {
            ...
            if (audioMgr.ptNetwork.nSpecialCase ==1){
                //DEBUG_PRINT("[Audio mgr] thread goto stop #line %d \n",__LINE__);
                goto stop;
            }
...
//                smtkAudioMgrStop();
            ...
            //DEBUG_PRINT("[Audio mgr] sem wait eof #line %d \n",__LINE__);

            sem_wait(gpAudioMgrSemaphore);

            //DEBUG_PRINT("[Audio mgr] sem_wait exit #line %d \n",__LINE__);
            PalAssert(nResult == 0);
            LOG_DEBUG
...
static int audio_special_case_play(){
    ...
    if (i2s_get_DA_running && audioMgr.ptNetwork.nSpecialCase ==1){
        ...
        DEBUG_PRINT("[Audio mgr] audio_special_case_play da %d ,read pointer %d write pointer %d %d\n",128*1024,gOutReadPointer,gOutWritePointer,audioMgr.ptNetwork.nLocalFileSize);
        if (gOutReadPointer == 0) {
#if (CFG_CHIP_FAMILY == 9850)
            i2s_mute_volume(1);//try to avoid pop sound 
#endif
            dstBuffer = gOutBuffer + gOutReadPointer;
            copySize = audioMgr.ptNetwork.nLocalFileSize;
            i2sNewWP = gOutReadPointer+audioMgr.ptNetwork.nLocalFileSize;
        } else if (gOutReadPointer+audioMgr.ptNetwork.nLocalFileSize<= 128*1024) {
            dstBuffer = gOutBuffer + gOutReadPointer;
            copySize = audioMgr.ptNetwork.nLocalFileSize;
            i2sNewWP = gOutReadPointer+audioMgr.ptNetwork.nLocalFileSize;
        } else if (gOutReadPointer+audioMgr.ptNetwork.nLocalFileSize > 128*1024) {
            ...
            copySize = audioMgr.ptNetwork.nLocalFileSize;
            i2sNewWP = audioMgr.ptNetwork.nLocalFileSize;
        } else {
        ...
        if (copySize)
        {
            memcpy(dstBuffer, audioMgr.sampleBuf, copySize);
#if CFG_CPU_WB
    ...
    us_for_sample = 1000/(nSampeRate/1000); // us for 1 sample    
    //printf("play time: %d\n", audioMgr.ptNetwork.nLocalFileSize/2*us_for_sample);
    usleep(audioMgr.ptNetwork.nLocalFileSize/2*us_for_sample);
    DEBUG_PRINT("[Audio mgr] audio_special_case_play set wp da %d ,read pointer %d write pointer %d\n",128*1024,gOutReadPointer,gOutWritePointer);
