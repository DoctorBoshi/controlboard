...
static const uint16_t kGenFuncTable[kGenFuncTableSize] = { 256, 485, 786, 1126,
		1484, 1849, 2217, 2586, 2955, 3324, 3693, 4063, 4432, 4801, 5171, 5540,
		5909, 6279, 6648, 7017, 7387, 7756, 8125, 8495, 8864, 9233, 9603, 9972,
		10341, 10711, 11080, 11449, 11819, 12188, 12557, 12927, 13296, 13665,
		14035, 14404, 14773, 15143, 15512, 15881, 16251, 16620, 16989, 17359,
		17728, 18097, 18466, 18836, 19205, 19574, 19944, 20313, 20682, 21052,
		21421, 21790, 22160, 22529, 22898, 23268, 23637, 24006, 24376, 24745,
		25114, 25484, 25853, 26222, 26592, 26961, 27330, 27700, 28069, 28438,
		28808, 29177, 29546, 29916, 30285, 30654, 31024, 31393, 31762, 32132,
		32501, 32870, 33240, 33609, 33978, 34348, 34717, 35086, 35456, 35825,
		36194, 36564, 36933, 37302, 37672, 38041, 38410, 38780, 39149, 39518,
		39888, 40257, 40626, 40996, 41365, 41734, 42104, 42473, 42842, 43212,
		43581, 43950, 44320, 44689, 45058, 45428, 45797, 46166, 46536, 46905 };
...
int32_t WebRtcAgc_CalculateGainTable(int32_t *gainTable, // Q16
		...
		uint8_t limiterEnable, int16_t analogTarget) // Q0
{
...
//    kLog10_2 = 49321; // 10*log10(2)  in Q14
	...
	tmp16no1 += WebRtcSpl_DivW32W16ResW16(tmp32no1 + (kCompRatio >> 1),
			kCompRatio);
	...
	diffGain = WebRtcSpl_DivW32W16ResW16(tmp32no1 + (kCompRatio >> 1),
			kCompRatio);
	...
	limiterLvlX = analogTarget - limiterOffset;
	limiterIdx = 2
			+ WebRtcSpl_DivW32W16ResW16((int32_t) limiterLvlX << 13,
					kLog10_2 / 2);
	tmp16no1 = WebRtcSpl_DivW32W16ResW16(limiterOffset + (kCompRatio >> 1),
			kCompRatio);
...
int32_t WebRtcAgc_AddFarendToDigital(DigitalAgc* stt, const int16_t* in_far,
		size_t nrSamples) {
	assert(stt != NULL);
	// VAD for far end
...
int32_t WebRtcAgc_ProcessDigital(DigitalAgc* stt,
		const int16_t* const * in_near, size_t num_bands, int16_t* const * out,
	...
	for (k = 0; k < 10; k++) {
		...
		stt->capacitorFast = AGC_SCALEDIFF32(-1000, stt->capacitorFast,
				stt->capacitorFast);
		...
		if (env[k] > stt->capacitorSlow) {
			...
			stt->capacitorSlow = AGC_SCALEDIFF32(500,
					(env[k] - stt->capacitorSlow), stt->capacitorSlow);
		} else {
			// decrease capacitorSlow
			stt->capacitorSlow = AGC_SCALEDIFF32(decay, stt->capacitorSlow,
					stt->capacitorSlow);
		}
		...
		size_t nrSamples)  // (i) number of samples
{
	...
	state->meanLongTerm = WebRtcSpl_DivW32W16ResW16(tmp32,
			WebRtcSpl_AddSatW16(state->counter, 1));
