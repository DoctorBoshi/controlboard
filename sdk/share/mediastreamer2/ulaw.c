...
static int offset8bit_to_16bit(unsigned char bit8_val){
    int bit16_val;
    bit16_val = (bit8_val - 128) << 8;
    return bit16_val;
}

static void offset8bit_dec_process(MSFilter *obj){
    ...
    while((m=ms_queue_get(obj->inputs[0]))!=NULL){
        ...
        mblk_meta_copy(m, o);
        for(;m->b_rptr<m->b_wptr;m->b_rptr++,o->b_wptr+=2){
            *((int16_t*)(o->b_wptr))=offset8bit_to_16bit(*m->b_rptr);
        }
        freemsg(m);
        ms_queue_put(obj->outputs[0],o);
    }    
...
MSFilterDesc ms_offset8bit_dec_desc={
    MS_OFFSET8BIT_DEC_ID,
    "MSOFFSET8BITDec",
    N_("offset8bit decoder"),
    MS_FILTER_DECODER,
    "offset8bit",
    1,
    1,
...
MSFilterDesc ms_offset8bit_dec_desc={
    .id=MS_OFFSET8BIT_DEC_ID,
    .name="MSOFFSET8BITDec",
    .text=N_("offset8bit decoder"),
    .category=MS_FILTER_DECODER,
    .enc_fmt="offset8bit",
    .ninputs=1,
    .noutputs=1,
    .process=offset8bit_dec_process,
};

#endif

MS_FILTER_DESC_EXPORT(ms_offset8bit_dec_desc)
