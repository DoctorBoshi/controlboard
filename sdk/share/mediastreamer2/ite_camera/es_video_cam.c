...
static VIDEO_INFO video_use_info[256] = {0};
    ...
    MSFilter    *f)
{
    ...
    do
    {
        ...
        VideoEncoder_GetSample(&vidSample, k);
    ...
    do
    {
        ...
        for (i = 0; i < use_cnt; i++)
        {
            if (video_use_info[i].bused)
                break;
            else if (i == (use_cnt - 1) && !video_use_info[i].bused)
            {
    #ifdef CFG_XCPU_MSGQ
                VideoEncoder_Close();
    #endif
