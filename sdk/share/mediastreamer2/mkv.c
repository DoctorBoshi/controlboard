...
static void H264Private_append(const H264Private *obj, const MSList *spsList, const MSList *ppsList) {
    ...
    ms_list_for_each2(spsList, (MSIterate2Func)_ms_list_append_copy_if_not_exist, &obj->sps_list);
    ms_list_for_each2(ppsList, (MSIterate2Func)_ms_list_append_copy_if_not_exist, &obj->pps_list);
}
...
static const ModuleDesc pcm_module_desc = {
    "A_PCM",      
    "A_PCM/INT/BIG",
    mu_law_module_new,
    mu_law_module_free,
...
static matroska_block *write_frame(MKVRecorder *obj, mblk_t *buffer, int pin) {
    ...
    {
        if(strcmp("PCMU", obj->inputDescsList[pin]->encoding) == 0 || strcmp("PCMA", obj->inputDescsList[pin]->encoding) == 0)
        {
            obj->select_flow = AudioFromRtpRecv;
        }
        else if(strcmp("A_PCM", obj->inputDescsList[pin]->encoding) == 0)
        {
    ...
    if(obj->openMode == MKV_OPEN_CREATE) {
        ...
        for(i=0; i < f->desc->ninputs; i++) {
            if(obj->inputDescsList[i] != NULL) {
                if(strcmp("PCMU", obj->inputDescsList[i]->encoding) == 0 || strcmp("PCMA", obj->inputDescsList[i]->encoding) == 0)
                {
                    obj->select_flow = AudioFromRtpRecv;
                }
                else if(strcmp("A_PCM", obj->inputDescsList[i]->encoding) == 0)
                {
                    obj->select_flow = AudioFromSoundRead;
                }
