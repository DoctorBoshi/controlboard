...
namespace details
{
struct daemon_item  // 記錄 HTTP daemon 的 data structure
{
    webserver         *ws;
    ...
    internal_comet_manager(new details::comet_manager())
{
    ...
    pthread_mutex_init(&cache_guard, NULL);
#endif
...
webserver::~webserver()
{
    ...
    pthread_mutex_destroy(&cache_guard);
#endif
...
bool webserver::register_resource(const std::string& resource, http_resource *hrm, bool family)
{
    ...
    pair<map<details::http_endpoint, http_resource *>::iterator, bool> result = registered_resources.insert(
         map<details::http_endpoint, http_resource *>::value_type(idx, hrm));   // 將一對 endpoint 和 resource 放進 registered_resources 列表中
...
bool webserver::start(bool blocking)
{
    ...
    // 這裡是設定 microhttpd library 所需要的參數
    iov.push_back(gen(MHD_OPTION_NOTIFY_COMPLETED,  (intptr_t)&request_completed));
    ...
    if (bind_address != 0x0) iov.push_back(gen(MHD_OPTION_SOCK_ADDR, (intptr_t)bind_address));
    if (bind_socket != 0)    iov.push_back(gen(MHD_OPTION_LISTEN_SOCKET, bind_socket));
    ...
    if (max_threads != 0)                   iov.push_back(gen(MHD_OPTION_THREAD_POOL_SIZE, max_threads));
    if (max_connections != 0)               iov.push_back(gen(MHD_OPTION_CONNECTION_LIMIT, max_connections));
    if (memory_limit != 0)                  iov.push_back(gen(MHD_OPTION_CONNECTION_MEMORY_LIMIT, memory_limit));
    if (per_IP_connection_limit != 0)       iov.push_back(gen(MHD_OPTION_PER_IP_CONNECTION_LIMIT, per_IP_connection_limit));
    if (max_thread_stack_size != 0)         iov.push_back(gen(MHD_OPTION_THREAD_STACK_SIZE, max_thread_stack_size));
    if (nonce_nc_size != 0)                 iov.push_back(gen(MHD_OPTION_NONCE_NC_SIZE, nonce_nc_size));
    if (use_ssl)                            iov.push_back(gen(MHD_OPTION_HTTPS_MEM_KEY, 0, (void *)https_mem_key.c_str()));
    if (use_ssl)                            iov.push_back(gen(MHD_OPTION_HTTPS_MEM_CERT, 0, (void *)https_mem_cert.c_str()));
    if (https_mem_trust != "" && use_ssl)   iov.push_back(gen(MHD_OPTION_HTTPS_MEM_TRUST, 0, (void *)https_mem_trust.c_str()));
    if (https_priorities != "" && use_ssl)  iov.push_back(gen(MHD_OPTION_HTTPS_PRIORITIES, 0, (void *)https_priorities.c_str()));
    if (digest_auth_random != "")           iov.push_back(gen(MHD_OPTION_DIGEST_AUTH_RANDOM, digest_auth_random.size(), (char *)digest_auth_random.c_str()));
#ifdef HAVE_GNUTLS
    if (cred_type != http_utils::NONE)      iov.push_back(gen(MHD_OPTION_HTTPS_CRED_TYPE, cred_type));
#endif  //HAVE_GNUTLS

    iov.push_back(gen(MHD_OPTION_END, 0, NULL));

    int start_conf = start_method;
    if (use_ssl)        start_conf |= MHD_USE_SSL;
    if (use_ipv6)       start_conf |= MHD_USE_IPv6;
    if (debug)          start_conf |= MHD_USE_DEBUG;
    if (pedantic)       start_conf |= MHD_USE_PEDANTIC_CHECKS;
    if (comet_enabled)  start_conf |= MHD_USE_SUSPEND_RESUME;
#ifdef USE_FASTOPEN
    start_conf   |= MHD_USE_TCP_FASTOPEN;
#endif

    this->running = true;   // 修改 running state

    struct MHD_Daemon *daemon = MHD_start_daemon(       // 要求 microhttpd 啟動一個 daemon
    ...
    bool                 value_onclose = false; // 是否已被關閉
    if (blocking)
    {
...
bool webserver::stop()
{
    ...
    pthread_mutex_lock(&mutexwait);
    this->running = false;  // 修改 running state
    ...
    const char         *arg_value)
{
    ...
    {
#ifdef WIN32
        int buf_size = strlen(key) + strlen(value) + 3;
    ...
    const char                     *method)
{
    ...
    bool                                   found = false;
    struct MHD_Response                    *raw_response = NULL;
    if (!single_resource)
    {
        ...
        if (fe == registered_resources_str.end())
        {
            ...
            if (regex_checking)
            {
                ...
                size_t len = 0;
                size_t tot_len = 0;
                for (it = registered_resources.begin();
    ...
    try
    {
        ...
        catch (const file_access_exception&)
        {
            not_found_page(&dhrs, mr);
            dhrs->get_raw_response(&raw_response, this);
        }
                                    ...
                                    size_t *upload_data_size, void **con_cls)
{
    ...
    return body ? static_cast<webserver*>(cls)->bodyfull_requests_answer_first_step(connection, mr) 
                : static_cast<webserver*>(cls)->bodyless_requests_answer(connection, method, version, mr);
}
