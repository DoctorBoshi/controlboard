    ...
    JCOMM_HANDLE *pHJComm)
{
    ...
    do
    {
        ...
        if (pJOutBufInfo_0->pBufAddr)
        {
			...
			if( (outInfo.width == ithLcdGetHeight()) && (outInfo.height ==  ithLcdGetWidth()) )//for rotate.
			{			
				if (outInfo.width > ithLcdGetHeight())
				    outInfo.width = ithLcdGetHeight();
				if (outInfo.height > ithLcdGetWidth())
				    outInfo.height = ithLcdGetWidth();
			}else
			{
	            if (outInfo.width > ithLcdGetWidth())
	                outInfo.width = ithLcdGetWidth();
	            if (outInfo.height > ithLcdGetHeight())
    ...
    void         *extraData)
{
    ...
    case JEL_CTRL_ISP_SET_DISP_INFO:
        if (extraData)
        {
			...
			if(value) 
				MjpegFullScreen = 0;  //Powei says don`t fullscreen it.
