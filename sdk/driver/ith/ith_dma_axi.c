...
int ithDmaMemcpylld(uint32_t src, uint32_t dst, uint32_t len);
...
int ithDmaMemcpy(uint32_t dst, uint32_t src, uint32_t len)
{
    ...
    if (len > ITH_DMA_CYC_MASK)
    {
        ...
        return ithDmaMemcpylld(src, dst, len);
    }
...
int ithDmaMemcpyAsync(uint32_t src, uint32_t dst, uint32_t len, ITHDmaAsyncHandler cb, void* arg)
{
    int ch;
    uint32_t ctrl, cfg;
    struct dma_mem_async data = { cb, arg };
    ...
    ch = ithDmaRequestCh("m2m2", ITH_DMA_CH_PRIO_HIGH, m2m2_isr, &data);
    if (ch < 0)
    {
        LOG_ERR "%s(0x%X, 0x%X,%d) no available dma channel \n", __func__, src, dst, len LOG_END
        return -1;
