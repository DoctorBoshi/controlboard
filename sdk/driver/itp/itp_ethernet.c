...
int MacRead(uint8_t *macaddr)
{
    ...
    int fd = open(":sd:0", O_RDWR, 0);
    LOG_DBG "sd0 fd: 0x%X\n", fd LOG_END
#elif defined(CFG_NET_ETHERNET_MAC_ADDR_SD1)
    int fd = open(":sd:1", O_RDWR, 0);
    LOG_DBG "sd1 fd: 0x%X\n", fd LOG_END
#endif
    if (fd == -1)
    {
...
{
/*
** Only for indoor multi-interface.
** eason 05/23/2016
**
*/
    struct netif *xnetif_main, *xnetif_subnet_0, *xnetif_subnet_1;
    ...
    xnetif_subnet_1 = &ethNetifs[1];
	xnetif_main     = &ethNetifs[2];

/* interface main, it's the main interface of indoor doorbell */
    ...
    netif_set_default(netif_add(xnetif_main, &ipaddr, &netmask, &gw, NULL, ethernetif_init, tcpip_input)); //only set one interface for default
...
/* interface 0 */
    ...
    netif_add(xnetif_subnet_0, &ipaddr, &netmask, &gw, NULL, ethernetif_init, tcpip_input);
...
/* interface 1 */
    ...
    netif_add(xnetif_subnet_1, &ipaddr, &netmask, &gw, NULL, ethernetif_init, tcpip_input);
    netif_set_up(xnetif_subnet_1);
    ...
    dhcp_set_struct(xnetif_main, &ethNetifDhcp);
    dhcp_start(xnetif_main);
    ethDhcp = true;
...
static void EthernetReset(ITPEthernetSetting *setting)
{
    ...
    xnetif = &ethNetifs[setting->index]; //index_0 is interface_0, index_1 is interface_1, index_2 is interface_main
...
static int EthernetIoctl(int file, unsigned long request, void *ptr, void *info)
{
    ...
    case ITP_IOCTL_IS_AVAIL:
		...
		eth_netif[2] = ethNetifs[2].ip_addr.addr ? 1 : 0;

        return eth_netif[0] && eth_netif[1] && eth_netif[2];
#else
    ...
    case ITP_IOCTL_ENABLE:
        //netif_set_link_up(&ethNetifs[CFG_NET_ETHERNET_COUNT - 1]);
#ifdef CFG_NET_ETHERNET_MULTI_INTERFACE
        netif_set_link_up(&ethNetifs[0]);
        netif_set_link_up(&ethNetifs[1]);
        netif_set_link_up(&ethNetifs[2]);
#else
        netif_set_link_up(&ethNetifs[0]);
#endif
    ...
    case ITP_IOCTL_DISABLE:
        //netif_set_link_down(&ethNetifs[CFG_NET_ETHERNET_COUNT - 1]);
#ifdef CFG_NET_ETHERNET_MULTI_INTERFACE
        netif_set_link_down(&ethNetifs[0]);
        netif_set_link_down(&ethNetifs[1]);
        netif_set_link_down(&ethNetifs[2]);
#else
        netif_set_link_down(&ethNetifs[0]);
#endif
