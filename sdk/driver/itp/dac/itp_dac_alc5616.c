...
static int _alc5616_channel = 1; //note set 2 can get louder 
...
static void I2C_common_write_word(unsigned char RegAddr, unsigned short d)
{
    ...
    {
        if(0 == (flag = mmpIicSendData(IIC_PORT_0, IIC_MASTER_MODE, ALC5616_I2CADR, RegAddr, &tmp, 2)))
        {
        	success = 1;
...
static unsigned short I2C_common_read_word(unsigned char RegAddr)
{
    ...
    for(retry=0; retry<50; retry++)
    {
        if(0 == (flag = mmpIicReceiveData(IIC_PORT_0, IIC_MASTER_MODE, ALC5616_I2CADR, &RegAddr, 1, &d, 2)))
        {
            success = 1;
#ifdef ALC5616_DEBUG_I2C_COMMON_WRITE
