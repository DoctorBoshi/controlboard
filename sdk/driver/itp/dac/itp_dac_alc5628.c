...
static void I2C_common_write_word(unsigned char RegAddr, unsigned short d)
{
    ...
    {
        if(0 == (flag = mmpIicSendData(IIC_PORT_0, IIC_MASTER_MODE, ALC5628_I2CADR, RegAddr, &tmp, 2)))
        {
        	success = 1;
...
static unsigned short I2C_common_read_word(unsigned char RegAddr)
{
    ...
    for(retry=0; retry<50; retry++)
    {
        if(0 == (flag = mmpIicReceiveData(IIC_PORT_0, IIC_MASTER_MODE, ALC5628_I2CADR, &RegAddr, 1, &d, 2)))
        {
            success = 1;
#ifdef ALC5628_DEBUG_I2C_COMMON_WRITE
...
static void init_alc5628_common(void)
{
	...
	{
		printf("ALC5628# DAC is running, skip re-init process !\n");
		pthread_mutex_unlock(&ALC5628_MUTEX);
		return;
	}
	if(_alc5628_AD_running)
	{
		printf("ALC5628# ADC is running, skip re-init process !\n");
		pthread_mutex_unlock(&ALC5628_MUTEX);
		return;
	}
	...
	/* start programming ALC5628 */
	{
		...
		alc5628_write_reg(0x3C, 0x7FF8);
		alc5628_write_reg(0x3E, 0xF6F0);
		
		alc5628_write_reg(0x02, 0x0000); // SPK OUT VOL 0db
		alc5628_write_reg(0x04, 0x0000); // HP OUT VOL 0db
		alc5628_write_reg(0x0C, 0xFFFF); //DAC Digital Volume mute
...
static void deinit_alc5628_common(void)
{
	...
	if(_alc5628_DA_running)
	{
		printf("ALC5628# DAC is running, skip deinit !\n");
		pthread_mutex_unlock(&ALC5628_MUTEX);
		return;
	}
	if(_alc5628_AD_running)
	{
		printf("ALC5628# ADC is running, skip deinit !\n");
		pthread_mutex_unlock(&ALC5628_MUTEX);
		return;
	}
