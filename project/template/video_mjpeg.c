...
static uint32_t int_count =0 , dec_count=0 ,timeout_ms = 3000;
 ...
 void *arg)
{
	...
	uint32_t		frame_DataSize	=0, AVIH_Size=0;
	FILE 			*f = 0;
...
void PlayMjpeg(int x, int y, int width, int height, int bgColor, int volume)
{
	...
	pthread_create(&tidAVIOpen, NULL, AVIFileOpen, (void *)CFG_PUBLIC_DRIVE ":/media/" BOOT_VIDEO);
	usleep(1000*100);
